...
Before delivery, complete the customer-branding checklist in `Delivery Branding Guide.md`. The current HTML configuration is set to Boeing through `Student\Labs\Web\scripts\delivery-config.js`.
...
- Update `Student\Labs\Web\scripts\delivery-config.js` before each customer delivery and verify every HTML page.
